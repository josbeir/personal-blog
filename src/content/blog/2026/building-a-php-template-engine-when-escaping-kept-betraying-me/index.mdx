---
title: 'I Built a Full-Blown PHP Template Engine Faster Than I Expected (Because Escaping Kept Betraying Me)'
description: 'Why I built Sugar after fighting template engine edge cases, how context-aware escaping and readable syntax changed my workflow, and what PHP teams can learn from it.'
date: 2026-03-05
tags: [PHP, Template Engine, Escaping, Security, DX]
image: ./teaser.png
---

> **TL;DR** -- I built [Sugar](https://josbeir.github.io/sugar/), a PHP template engine that keeps your templates as real PHP files, adds directive-style attributes (`s:if`, `s:foreach`, `s:class`, ...), context-aware escaping, layout inheritance, and a component system -- without making you learn a new language. Check the [docs](https://josbeir.github.io/sugar/) if you want to skip straight to the code.
...
- `x-data` attributes with JSON-like payloads,
- partials that need scoped variables,
...
- `json_encode()` alone does not solve HTML attribute context.
- One quote mismatch and your front-end parser becomes an archaeologist.
...
So: SSR with directive-style ergonomics, sprinkling Alpine where interactivity is actually needed. That's a stack I can still feel good about in 2026.

## Why I Didn't Just "Use Less PHP"
...
1. **Application templates** (your team writes them).
2. **User-authored templates** (your users write them, e.g. Liquid-like scenarios).
...
- `s:bind` passes typed intent (props), not random attributes.
- non-prop attributes (like `class`, `x-data`) merge onto the root element.
- slots keep semantic control with caller-defined markup.
...
And if you want proof this isn't theoretical: **this very site is built with [Glaze](https://josbeir.github.io/glaze/)**, a static site generator I also wrote -- which uses Sugar as its template engine. The templates rendering these posts, the layout, the components -- all Sugar. Dogfooding at its most literal.
